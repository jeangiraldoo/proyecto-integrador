...
const SEED_RESPONSE = {
    overdue: [
        ...
        makeSubtask({ id: 1, name: 'Subtarea vencida A (más antigua)', target_date: TWO_DAYS_AGO, estimated_hours: 3 }),
        makeSubtask({ id: 2, name: 'Subtarea vencida B (ayer)', target_date: YESTERDAY, estimated_hours: 1 }),
    ],
    today: [
        // Menor esfuerzo primero (estimated_hours ASC)
        makeSubtask({ id: 3, name: 'Subtarea rápida (1h)', target_date: TODAY_ISO, estimated_hours: 1 }),
        makeSubtask({ id: 4, name: 'Subtarea media (2h)', target_date: TODAY_ISO, estimated_hours: 2 }),
        makeSubtask({ id: 5, name: 'Subtarea larga (4h)', target_date: TODAY_ISO, estimated_hours: 4, status: 'in_progress' }),
    ],
    upcoming: [
        // Más cercana primero (date ASC)
        makeSubtask({ id: 6, name: 'Subtarea próxima cercana', target_date: TOMORROW, estimated_hours: 2 }),
        makeSubtask({ id: 7, name: 'Subtarea próxima lejana', target_date: IN_5_DAYS, estimated_hours: 5 }),
        makeSubtask({ id: 8, name: 'Subtarea fuera de rango N', target_date: IN_10_DAYS, estimated_hours: 1 }),
    ],
    meta: { n_days: 7 },
};
...
test.describe('US-4 | Caso de éxito — Vista /hoy con datos controlados', () => {
    ...
    test('EX-02 | los tabs deben mostrar el conteo correcto de subtareas', async ({ page }) => {
        ...
        const overdueBadge = page.locator('button', { hasText: 'Vencidas' }).first().locator('span', { hasText: '2' });
        const todayBadge = page.locator('button', { hasText: 'Para hoy' }).first().locator('span', { hasText: '3' });
        const upcomingBadge = page.locator('button', { hasText: 'Próximas' }).first().locator('span', { hasText: '3' });
    ...
    test('EX-03 | debe activar automáticamente el tab de Vencidas si existen subtareas vencidas', async ({ page }) => {
        ...
        await expect(page.locator('p, div').filter({ hasText: 'Subtarea vencida A (más antigua)' }).first()).toBeVisible();
    });

    test('EX-04 | debe mostrar la barra de sort-hint con el criterio correcto por grupo', async ({ page }) => {
    ...
    test('EX-05 | debe mostrar el summary pill con el conteo de subtareas pendientes', async ({ page }) => {
        ...
        const summaryPill = page.locator('span, div').filter({ hasText: /subtarea.? pendiente/i }).first();
        await expect(summaryPill).toBeVisible();
    });

    test('EX-06 | el summary pill debe mostrar la advertencia de vencidas sin completar', async ({ page }) => {
    ...
    test('EX-09 | Vencidas debe mostrar subtareas ordenadas por fecha ASC (más antigua primero)', async ({ page }) => {
    ...
    test('EX-10 | Para hoy debe mostrar subtareas ordenadas por estimated_hours ASC (menor esfuerzo primero)', async ({ page }) => {
        await page.locator('button', { hasText: 'Para hoy' }).first().click();
    ...
    test('EX-11 | Próximas debe mostrar subtareas ordenadas por fecha ASC (más cercana primero)', async ({ page }) => {
        await page.locator('button', { hasText: 'Próximas' }).first().click();
    ...
    test('EX-16 | las cards vencidas deben mostrar badge de fecha en rojo ("hace Xd")', async ({ page }) => {
    ...
    test('EX-18 | las cards del tab "Próximas" deben mostrar badge de días positivo', async ({ page }) => {
        await page.locator('button', { hasText: 'Próximas' }).first().click();
        // Mañana → "Mañana", resto → "Xd"
        const badgeText = page.locator('span').filter({ hasText: /Mañana|\dd/ }).first();
        await expect(badgeText).toBeVisible();
    });

    test('EX-19 | las cards deben mostrar el badge de estado (Pendiente/En progreso/Completada)', async ({ page }) => {
        await expect(page.locator('span', { hasText: 'Pendiente' }).first()).toBeVisible();
    });

    // ── Interacción con tabs ──────────────────────────────────────────────

    test('EX-20 | hacer clic en "Para hoy" debe cambiar el contenido visible del panel', async ({ page }) => {
        await page.locator('button', { hasText: 'Para hoy' }).first().click();

        // Las subtareas de overdue ya no deben estar visibles
        await expect(page.locator('[role="button"]').filter({ hasText: 'Subtarea vencida A (más antigua)' })).toHaveCount(0);

        // Las subtareas de hoy sí
        await expect(page.locator('p, div').filter({ hasText: 'Subtarea rápida (1h)' }).first()).toBeVisible();
    });

    test('EX-21 | sub-caption del tab debe indicar cuántas subtareas están sin completar', async ({ page }) => {
    ...
    test('EX-22 | el filtro "En progreso" debe mostrar solo subtareas en ese estado', async ({ page }) => {
        ...
        await expect(page.locator('[role="button"]').filter({ hasText: 'Subtarea larga (4h)' })).toBeVisible();
        await expect(page.locator('[role="button"]').filter({ hasText: 'Subtarea rápida (1h)' })).toHaveCount(0);
    });

    test('EX-23 | el filtro "Todos" debe restaurar todas las subtareas visibles', async ({ page }) => {
        await page.locator('button', { hasText: 'Para hoy' }).first().click();
        ...
        await expect(page.locator('[role="button"]').filter({ hasText: 'Subtarea rápida (1h)' })).toBeVisible();
        await expect(page.locator('[role="button"]').filter({ hasText: 'Subtarea larga (4h)' })).toBeVisible();
    });
    ...
    test('EX-24 | hacer clic en una card debe abrir el panel de detalle', async ({ page }) => {
        const firstCard = page.locator('[role="button"]').filter({ hasText: 'Subtarea vencida A (más antigua)' }).first();
        await firstCard.click();

        // El panel lateral debe aparecer (contiene el nombre de la subtarea)
        const panel = page.locator('[role="dialog"], aside, .detail-panel, div').filter({ hasText: 'Subtarea vencida A (más antigua)' }).last();
    ...
    test('EX-26 | el badge de estado en una card debe abrir un dropdown al hacer clic', async ({ page }) => {
        // El badge de estado (ej: "Pendiente") dentro de la primera card funciona como botón
        const firstCard = page.locator('[role="button"]').first();
        const statusBadge = firstCard.locator('button', { hasText: /Pendiente|En progreso|Completada/ });
...
test.describe('US-4 | Caso vacío — sin subtareas urgentes', () => {
    ...
    test('VA-01 | debe mostrar mensaje de "todo libre" al no haber subtareas en el tab activo', async ({ page }) => {
        // Cada columna vacía muestra "Nada por aquí — ¡todo libre! 🎉"
        await expect(page.locator('p, div').filter({ hasText: /Nada por aquí|todo libre/ }).first()).toBeVisible({ timeout: 8000 });
    });

    test('VA-02 | el summary pill debe indicar que no hay subtareas urgentes', async ({ page }) => {
        // "Sin subtareas urgentes — ¡todo bajo control!"
        await expect(page.locator('span, div').filter({ hasText: /Sin subtareas urgentes|todo bajo control/ }).first()).toBeVisible({ timeout: 8000 });
    });

    test('VA-03 | el sub-caption de cada tab debe decir "todo completado"', async ({ page }) => {
        await expect(page.locator('span', { hasText: 'todo completado' }).first()).toBeVisible({ timeout: 8000 });
    });
...
test.describe('US-4 | Caso de error — API devuelve 500', () => {
    ...
    test('ER-02 | ante un error de red, la vista debe degradarse sin crashear', async ({ page }) => {
        ...
        await expect(page.locator('button', { hasText: /Vencidas|Para hoy|Próximas/ }).first()).toBeVisible({ timeout: 8000 });
    });
...
test.describe('US-4 | UX, diseño y accesibilidad', () => {
    ...
    test('UX-07 | la toolbar debe incluir: summary pill + botón Nueva subtarea + chips de filtro', async ({ page }) => {
        await expect(page.locator('button', { hasText: 'Nueva subtarea' })).toBeVisible();
        await expect(page.locator('button', { hasText: 'Todos' }).first()).toBeVisible();
        // Summary pill
        await expect(page.locator('span, div').filter({ hasText: /subtarea.? pendiente|Sin subtareas urgentes/i }).first()).toBeVisible();
    });

    test('UX-08 | la sort-hint bar debe mostrar la etiqueta "Orden" en mayúsculas', async ({ page }) => {
        await expect(page.locator('span', { hasText: 'Orden' })).toBeVisible();
    });

    test('UX-09 | el tab activo debe mostrar la barra de acento en la parte superior', async ({ page }) => {
    ...
    test('UX-10 | la página debe cargar en menos de 5 segundos', async ({ page }) => {
        ...
        await expect(page.locator('button', { hasText: 'Para hoy' }).first()).toBeVisible({ timeout: 5000 });
        const elapsed = Date.now() - start;
        expect(elapsed).toBeLessThan(5000);
    });

    test('UX-11 | el título de la página debe identificar la sección (title tag)', async ({ page }) => {
    ...
    test('UX-13 | cambiar de tab debe actualizar la sort-hint bar con el criterio correcto', async ({ page }) => {
        // Vencidas → antiguas
        await expect(page.locator('text=más antiguas primero')).toBeVisible();
    ...
    test('UX-14 | hacer clic en "Nueva subtarea" debe abrir un modal o panel de creación', async ({ page }) => {
    ...
    test('UX-15 | el filtro activo debe tener indicador visual distinto (estilo activo)', async ({ page }) => {
    ...
    test('UX-16 | cerrar el dropdown de estado haciendo clic fuera debe funcionar', async ({ page }) => {
        const firstCard = page.locator('[role="button"]').first();
        const statusBadge = firstCard.locator('button', { hasText: /Pendiente|En progreso|Completada/ });
...
test.describe('US-4 | Integración real — backend Django + base de datos', () => {
    ...
    test('INT-01 | la vista /hoy debe responder con éxito desde el backend real', async ({ page }) => {
        // Verificar que la petición al backend devuelva 200
        const [response] = await Promise.all([
            page.waitForResponse(r => r.url().includes('/today') || r.url().includes('planner'), { timeout: 10000 }).catch(() => null),
            page.reload(),
        ]);
    ...
    test('INT-02 | debe renderizar al menos uno de los tres estados posibles (éxito, vacío, error)', async ({ page }) => {
        // Éxito: tabs visibles. Vacío: "todo libre". Error: degradación.
        const tabsVisible = await page.locator('button', { hasText: /Vencidas|Para hoy|Próximas/ }).first().isVisible().catch(() => false);
        const emptyVisible = await page.locator('text=/Nada por aquí|Sin subtareas urgentes/').isVisible().catch(() => false);

        expect(tabsVisible || emptyVisible).toBe(true);
    });

    test('INT-03 | el frontend debe enviar el header de Authorization en la petición', async ({ page }) => {
    ...
    test('INT-04 | la respuesta de la API debe contener las claves overdue, today, upcoming y meta', async ({ page }) => {
